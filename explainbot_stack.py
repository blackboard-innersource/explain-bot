...
from aws_cdk import aws_apigatewayv2_integrations as _a2int
...
from aws_cdk import custom_resources as _resources
...
class ExplainBotLambdaStack(cdk.Stack):
    ...
    def __init__(
        self, scope: cdk.Construct, construct_id: str, stage: str, **kwargs
    ) -> None:
        ...
        **kwargs
        ...
        acronym_denied_table = _dynamo.Table(
            ...
            partition_key=_dynamo.Attribute(
                name="Acronym", type=_dynamo.AttributeType.STRING
            ),
            sort_key=_dynamo.Attribute(
                name="Deleted_at", type=_dynamo.AttributeType.STRING
            ),
        ...
        explain_bot_lambda.add_environment(
            "TABLE_DENIED_NAME", acronym_denied_table.table_name
        )
        add_meaning_lambda.add_environment(
            "TABLE_DENIED_NAME", acronym_denied_table.table_name
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        acronym_table.add_global_secondary_index(
            partition_key=_dynamo.Attribute(
                name="Approval", type=_dynamo.AttributeType.STRING
            ),
...
class ExplainSlackBotStack(cdk.Stack):
    def __init__(
        ...
        ExplainBotCloudWatchStack(
            self, "ReminderStack", stage=stage, table=database_stack.table
        )
